...
Shader "Aubergine/Object/NoLights/Water/ProMedium" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			CGPROGRAM
				...
				fixed4 _WaveTex_ST;
				fixed _WaveSpeed, _FresnelBias, _Refraction;
				...
				struct v2f {
					...
					fixed3 T2W0 : TEXCOORD3;
					fixed3 T2W1 : TEXCOORD4;
					fixed3 T2W2 : TEXCOORD5;
				};
				...
				fixed4 frag(v2f i) : COLOR {
					...
					fixed fresnel = max(_FresnelBias + (1.0 - _FresnelBias) * pow(facing, 5.0), 0.0);
					//fixed fresnel = 1 / pow(1 + EdotN, 5);

					fixed3 deepCol = (refraction * _Refraction + _Color * (1 - _Refraction));
					fixed3 waterCol = (_Color * facing + deepCol * (1.0 - facing));
					fixed3 color = fresnel * reflection + waterCol;
