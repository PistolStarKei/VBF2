...
public class LureShop_Menu_Line :  TackeMenuParent {
    ...
    public override void  OnBtnBuy(){
        Debug.Log(GetMethodName()+this.name);
            ...
            if(DataManger.Instance.canBuyDolller(GetPrice(currentSelect))){
                ...
                DataManger.Instance.GAMEDATA.lureHas_Line[GetTittle(currentSelect)]++;
                CheckEarlyAccess();
                AudioManager.Instance.BuyAndEquip();
                SetContents(currentSelect);
                itemList.items[Mathf.FloorToInt(currentSelect/2.0f)].GetItem(currentSelect%2).SetNums(GetHasNum(currentSelect)
    ...
    public  override void SetContents(int i){
        ...
        //個数　レンジ　特殊能力　ルアーのみ
        affecter.SetNum(GetHasNum(currentSelect), GetMaxSlots(i));
        affecter.SetRangeValue();
        SetAvilities(GetSPAvility(i));

        //レーダー
        if(GetHasNum(currentSelect)>0){
    ...
    private string GetSPAvility(int current){
