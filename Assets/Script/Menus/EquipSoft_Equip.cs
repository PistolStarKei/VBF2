...
public class EquipSoft_Equip : TackeMenuParentEquip {

    public void OnTappedRig(int rigNum){
        Debug.LogError("実装する　リグのたっぷ");
        int[] adons=GetAvaillableAdons(currentSelect);
        if(!isHas(currentSelect) || !getHasRig(adons[rigNum])){
            Debug.LogError("持っていない");
        }else{
            ...
            rods2.OnSelect_Soft(currentSelect);
        }

    }
    public EquipMenu_Rod rods2;
    ...
    public override void OnTappedItem(int num,bool isInit){
       ...
       SetContents(num);
        ...
        if(!isInit)rods2.OnSelect_Soft(num);
        //0 1 0
        //2 3 1
    }
        ...
        for(int i=0;i<GetItemSpriteLength()/2;i++){
            ...
            if(num< GetItemSpriteLength()){
                ...
                if(eqp>0){
                    ...
                    itemList.items[i].SetItems(num, GetTittle(num), GetHasNum(num),GetAvaillableNum(num),DataManger.Instance.HowManyEquipped_Lure(num,true), 
    ...
    public  override void SetContents(int i){
        ...
        Debug.Log("Adoons rig"+ currentRig);

       
        for(int e=0;e<5;e++){

            if(e>=adons.Length){
                rods2.affecter2.SetAdonItemsToUA(e);
            }else{
                if(e==0){
                    rods2.affecter2.SetAdonItems(e,Constants.RigDatas.itemTittles[adons[e]],"",
                    ...
                    rods2.affecter2.SetAdonItems(e,Constants.RigDatas.itemTittles[adons[e]],GetRigDesc(adons[e]),
                        Constants.RigDatas.itemPrices[adons[e]].ToString()+"$",Constants.RigDatas.itemSprites[adons[e]],getHasRig(adons[e]),currentRig==e?true:false);
    ...
    public int GetAvaillableNum(int current){
        return DataManger.Instance.GAMEDATA.lureHas_Soft_Temp[ GetTittle(current)];
    }
}
