...
public class RodItemListDynamic : MonoBehaviour {
    void Start(){
    ...
    public List<RodItemContainer> items=new List<RodItemContainer>();
    ...
    void FixedUpdate(){
        ...
        if(view.gameObject.activeSelf){
            if( view.verticalScrollBar.value<=0.2f){
                //top 下がまだある
                ShowIfNot(topIndecater,false);
                ShowIfNot(btmIndecater,true);
            }else{
                ShowIfNot(topIndecater,true);
                if(view.verticalScrollBar.value>=0.8f){
                    //btm
                    ShowIfNot(btmIndecater,false);
                }else{
                    ShowIfNot(btmIndecater,true);
    ...
    public void SpawnRow(){
        ...
        items.Add(current.gameObject.GetComponent< RodItemContainer>());
    }
}
