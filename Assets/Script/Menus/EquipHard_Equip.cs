...
public class EquipHard_Equip : TackeMenuParentEquip{
    ...
    public override void OnTappedItem(int num,bool isInit){
        ...
        SetContents(num);
        if(!isInit)rods2.OnSelect_Hard(num);
        //0 1 0
        //2 3 1
    }
        ...
        for(int i=0;i<GetItemSpriteLength()/2;i++){
            ...
            if(num< GetItemSpriteLength()){
                ...
                if(eqp>0){
                    ...
                    itemList.items[i].SetItems(num,  GetTittle(num), GetHasNum(num),GetAvaillableNum(num),DataManger.Instance.HowManyEquipped_Lure(num,false), 
    ...
    public int GetAvaillableNum(int current){
        return DataManger.Instance.GAMEDATA.lureHas_Hard_Temp[ GetTittle(current)];
    }
