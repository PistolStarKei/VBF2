...
public class EquipMenu_Rod : TackeMenuParent {

    public SoubiBtn soubiBtn;
    public void OnEquip(){
        Debug.Log("OnEquip"+currentSelect);
        AudioManager.Instance.Equip();
        WaitAndCover.Instance.ShowInfoList(Localization.Get("InfoEquipped")+GetTittle(currentSelect));
        StartCoroutine(EquipAffect());
    }

    public IEnumerator EquipAffect(){
        TackleParams.Instance.UpdateCurrentTackle(currentSelect);
        Debug.Log("OnEquip 1 "+currentSelect);
    ...
    IEnumerator SetRodToMainInvoke(){
        ...
        WaitAndCover.Instance.CoverAll(true);
        yield return null;
        Debug.Log("SetRodToMainInvoke 1");
    ...
    public LureParamsAffecterEquip affecter2;
    ...
    public  override void SetContents(int i){
        ...
        //個数　レンジ　特殊能力　ルアーのみ
        affecter2.SetNum(GetHasNum(i), GetMaxSlots(i));
        affecter2.SetRangeValue();
        SetAvilities(GetSPAvility(i));

        //レーダー
        if(isHasOrAvaillable(i)){

            if(TackleParams.Instance.currentTackle.name==Constants.RodsDatas.itemTittles[i]){
                soubiBtn.SetEnabled(false);
            }else{
            ...
            affecter2.mainRodToggle.SetActive(true,true);

            affecter2.mainRodToggle.SetToggle( DataManger.Instance.GAMEDATA.tackleSlots[i].isMainTackle);

            if(DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum==-1){
                //何もない
                if(DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum==-1){
                    affecter2.SetEquipped("","","","","","");
                    affecter2. SetAishou(-100.0f,-100.0f,-100.0f);
                }else{
                    affecter2.SetEquipped("","","","",Constants.LineDatas.itemSprites[DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum],Constants.LineDatas.itemTittles[DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum]);

                    affecter2. SetAishou(-100.0f,-100.0f,GetAishouLine(i,DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum));
                }
                ...
                if(DataManger.Instance.GAMEDATA.tackleSlots[i].isSoft){
                    ...
                    if(DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum==-1){
                        affecter2.SetEquipped("","",Constants.SoftLureDatas.itemSprites[DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum],
                    ...
                    affecter2. SetAishou(-100.0f,GetAishouLure(i,DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum,true),GetAishouLine(i,DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum));

                }else{

                    if(DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum==-1){
                        affecter2.SetEquipped(Constants.LureDatas.itemSprites[DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum],Constants.LureDatas.itemTittles[DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum],"","","","");
                    }else{
                        affecter2.SetEquipped(Constants.LureDatas.itemSprites[DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum],Constants.LureDatas.itemTittles[DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum],"","",Constants.LineDatas.itemSprites[DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum],Constants.LineDatas.itemTittles[DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum]);
                    }
                    affecter2. SetAishou(GetAishouLure(i,DataManger.Instance.GAMEDATA.tackleSlots[i].lureNum,false),-100.0f,GetAishouLine(i,DataManger.Instance.GAMEDATA.tackleSlots[i].lineNum));
                }

            }
            //affecter2.SetEquipped(string spriteName_Hard,string tittle_Hard,string spriteName_Soft,string tittle_Soft,string spriteName_Line,string tittle_Line);

        }else{
            affecter2.guiSetter.OnNA();
            soubiBtn.SetEnabled(false);
            Debug.Log("持っていない");
            affecter2.mainRodToggle.SetActive(true,false);
            affecter2.mainRodToggle.SetToggle(false);
            affecter2.NotBuyedORNoLure(true);
        }
    ...
    public override bool isHasOrAvaillable(int num){
            if(   DataManger.Instance.GAMEDATA.lureHas_Rods[ GetTittle(num)]>0){
                return true;
            }
        return false;
    }
    ...
    public EquipLine_Equip equipLine;
    public EquipSoft_Equip equipSoft;
    public EquipHard_Equip equipHard;

    public EquipBtn_Equip equipBtn;
    ...
    public void OnSelect_Soft(int i){
       ...
       equipBtn.SetKouho(Constants.SoftLureDatas.itemTittles[currentChangeSelect],Constants.SoftLureDatas.itemSprites[currentChangeSelect],equipSoft.itemAtlas
            ...
            isEquippable( DataManger.Instance.GAMEDATA.lureHas_Soft_Temp[Constants.SoftLureDatas.itemTittles[currentChangeSelect]],
    ...
    public void OnSelect_Hard(int i){
        ...
        equipBtn.SetKouho(Constants.LureDatas.itemTittles[currentChangeSelect],Constants.LureDatas.itemSprites[currentChangeSelect],equipHard.itemAtlas,
            isEquippable( DataManger.Instance.GAMEDATA.lureHas_Hard_Temp[Constants.LureDatas.itemTittles[currentChangeSelect]],
